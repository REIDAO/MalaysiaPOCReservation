...
contract('All', function(accounts) {
  ...
  it("Set State Refund", function() {
    return ReservationInstance.setStateRefund()
    .then(function(result) {
    })
    ;
  });

  it("Send Refund Transaction", function() {
    web3.eth.sendTransaction({from:accounts[3], to:ReservationInstance.address, value: 0, gas: 150000});
  });

  it("Verify totalEtherRefunded", function() {
    return ReservationInstance.totalEtherRefunded()
    .then(function(result) {
      assert.equal(2 * Math.pow(10,18), result.valueOf());
    })
    ;
  });

  it("Verify totalEtherEmergencyRefunded", function() {
    return ReservationInstance.totalEtherEmergencyRefunded()
    .then(function(result) {
