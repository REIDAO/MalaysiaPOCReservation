...
contract('All', function(accounts) {
  ...
  it("Set State Emergency", function() {
    return ReservationInstance.setStateEmergency("", {from:accounts[1]})
    .then(function(result) {
      ...
      event.watch(function(error, result) {
        if (result) {
          ...
          if (counter==1) {
            ...
            ReservationInstance.setStateEmergency(result.args.msg, {from:accounts[2]});
          } else {
            console.log("event2=" + result.args.status);
            console.log("event2=" + result.args.msg);
            event.stopWatching();
            ReservationInstance.state.call()
            .then(function(result) {
